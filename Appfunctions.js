...
export function sumCost(incart, products) {
  return incart?.reduce((acc, { id, qty }) => (acc + qty) * products.price, 0)
}
...
export function getStockByColorAndSize(color, size, product) {
  return product?.sizes
    .find((x) => x.size === size)
    ?.colors.find((x) => x.color === color)?.stock
}
...
export function getRating(reviews) {
  const rating =
    reviews?.reduce((n, { rating }) => n + rating, 0) / reviews?.length
...
export function determineInArray(array, color, size, product) {
  return array?.some(
    (x) => x.id === product.id && x.color === color && x.size === size
  )
}
export function totalPrice(incart, allproducts, appliedcodes, percentoff) {
  const totalwithoutcoupons =
    incart?.reduce(
      (acc, { id, qty }) =>
        acc +
        qty *
          allproducts
            .filter((x) => incart?.some((el) => x.id === el.id))
            .find((x) => x.id === id)?.price,
      0
    ) *
    ((100 - percentoff) / 100)
  const couponsdiscounts = appliedcodes?.reduce(
    (n, { discount }) => n + discount,
    0
  )
...
export function totalPriceWithTax(
  incart,
  allproducts,
  appliedcodes,
  percentoff,
  ...
  return (
    totalPrice(incart, allproducts, appliedcodes, percentoff) * (1 + tax) +
    (shipping ?? 0)
  )
}
...
export function getDiscountDollarFromCode(allproducts, code, incart) {
  return (
    getInCartProducts(allproducts, incart)
      .filter((x) => x.coupons.find((x) => x.couponcode === code))[0]
      .coupons.find((x) => x.couponcode === code).discount *
    getInCartProducts(allproducts, incart).filter((x) =>
      x.coupons.find((x) => x.couponcode === code)
    ).length
  )
}
...
export function sortArr(array, sort) {
  let bucketed = array.reduce((acc, x) => {
    ...
    let currentVals
    if (sort === "date") {
      currentVals = acc?.hasOwnProperty(
        convertDateToString(pivot.toDate(), true)
      )
        ? acc[convertDateToString(pivot.toDate(), true)]
        : []
    } else {
      currentVals = acc?.hasOwnProperty(pivot) ? acc[pivot] : []
    }
    currentVals.push(x)
    acc[sort === "date" ? convertDateToString(pivot.toDate(), true) : pivot] =
      currentVals
    return acc
  }, {})
  return bucketed
